...
namespace cyme{
    ...
    /** Implements fma function for cyme::vec_simd, lhs*mhs+rhs */
    ...
    /** Implements fma function for cyme::vec_simd, avoid useless computation  */
    template<class T,cyme::simd O, int N>
    forceinline vec_simd<T,O,N> muladd(const vec_simd<T,O,N>&, const ZERO, const vec_simd<T,O,N>& rhs){
        return rhs;
    }

    /** Implements fma function for cyme::vec_simd, avoid useless computation */
    template<class T,cyme::simd O, int N>
    forceinline ZERO muladd(const vec_simd<T,O,N>&, const ZERO, const ZERO){
        return ZERO();
    }
