...
BOOST_AUTO_TEST_CASE(bd2hkth1d4hkth3d4_test){
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<> dis(0, std::log(2));
    for (int i = 0; i < 100; ++i){
        double x = dis(gen);
        double y = poly::bruteforce<poly::coeffP2_5>(x)*poly::horner_kth<poly::coeffP4_1,1>(x)*poly::horner_kth<poly::coeffP4_2,3>(x);
