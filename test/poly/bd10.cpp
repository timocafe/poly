...
BOOST_AUTO_TEST_CASE(bd10_test){
    ...
    for (int i = 0; i < 100; ++i){
        ...
        x -= ((double)((int)sse_floor(1.4426950408889634 * x)))*0.6931471805599453;
        double y = poly::bruteforce<poly::coeffP10>(x)* (*(double *)(&twok));
        error = std::abs(y-ref)/ref;
        std::cout << x << ref << " " << y << " " << error << std::endl;
