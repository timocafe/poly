...
namespace poly{
    ...
    struct helper_horner{
        static inline cyme::vec_simd<T,O,N> horner(cyme::vec_simd<T,O,N> const& x){
            #ifdef __FMA__
            return muladd(x,helper_horner<T,O,N,C,n+m,m>::horner(x),cyme::vec_simd<T,O,N>(C<T,n>::coefficient()));
            #else
...
#ifdef __FMA__
            return muladd(cyme::pow<T,O,N,k-1>(x),
                          helper_horner<T,O,N,C,k-1,m>::horner(cyme::pow<T,O,N,m>(x)),
