...
BOOST_AUTO_TEST_CASE(bd2ed2ed2hkth1d2hkth1d2_test){
    ...
    for (int i = 0; i < 100; ++i){
        double x = dis(gen);
        double y = poly::bruteforce<poly::coeffP2_1>(x)*poly::estrin<poly::coeffP2_2>(x)*poly::estrin<poly::coeffP2_3>(x)*poly::horner_kth<poly::coeffP2_4,1>(x)*poly::horner_kth<poly::coeffP2_5,1>(x);
