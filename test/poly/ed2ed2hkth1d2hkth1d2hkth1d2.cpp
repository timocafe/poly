...
BOOST_AUTO_TEST_CASE(ed2ed2hkth1d2hkth1d2hkth1d2_test){
    ...
    for (int i = 0; i < 100; ++i){
        double x = dis(gen);
        double y = poly::estrin<poly::coeffP2_1>(x)*poly::estrin<poly::coeffP2_2>(x)*poly::horner_kth<poly::coeffP2_3,1>(x)*poly::horner_kth<poly::coeffP2_4,1>(x)*poly::horner_kth<poly::coeffP2_5,1>(x);
