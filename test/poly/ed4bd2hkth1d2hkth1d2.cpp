...
BOOST_AUTO_TEST_CASE(ed4bd2hkth1d2hkth1d2_test){
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<> dis(0, std::log(2));
    for (int i = 0; i < 100; ++i){
        double x = dis(gen);
        double y = poly::estrin<poly::coeffP4_1>(x)*poly::bruteforce<poly::coeffP2_3>(x)*poly::horner_kth<poly::coeffP2_4,1>(x)*poly::horner_kth<poly::coeffP2_5,1>(x);
