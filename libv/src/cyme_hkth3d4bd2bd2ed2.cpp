...
namespace cyme {
    ...
    struct experiment_exp{
        static forceinline vec_simd<T,O,N> exp(vec_simd<T,O,N> x){
            ...
            x = poly::horner_kth<T,O,N,poly::coeffP4_1,3>(x)*poly::bruteforce<T,O,N,poly::coeffP2_3>(x)*poly::bruteforce<T,O,N,poly::coeffP2_4>(x)*poly::estrin<T,O,N,poly::coeffP2_5>(x);
            /* p = 2^k; */
            p = twok<T,O,N>(k);
            /* e^x = 2^k * e^y */
            x *= p;
