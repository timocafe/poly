...
BOOST_AUTO_TEST_CASE(test_bd2bd2bd2bd2hkth1d2_test){
    ...
    for (int i = 0; i < 1000; ++i){
        ...
        double y = poly::bruteforce<poly::coeffP2_1>(x)*poly::bruteforce<poly::coeffP2_2>(x)*poly::bruteforce<poly::coeffP2_3>(x)*poly::bruteforce<poly::coeffP2_4>(x)*poly::horner_kth<poly::coeffP2_5,1>(x)* (*(double *)(&twok));
    ...
    rms/=100;
    std::cout << "test_bd2bd2bd2bd2hkth1d2" << " rms: "  << std::sqrt(rms) << " ulp: " << max << std::endl;
}
