...
namespace cyme {
    ...
    struct experiment_exp{
        static forceinline vec_simd<T,O,N> exp(vec_simd<T,O,N> x){
            ...
            x = poly::horner_kth<T,O,N,poly::coeffP2_5,1>(x)*poly::horner_kth<T,O,N,poly::coeffP4_1,2>(x)*poly::horner_kth<T,O,N,poly::coeffP4_2,3>(x);
            /* p = 2^k; */
            p = twok<T,O,N>(k);
            /* e^x = 2^k * e^y */
            x *= p;
